...
def ajuster_consommation(y_pred, df_test):
    ...
    # Facteur température
    temp_factor = 1.0 + 0.09 * (df_test["temperature"] - df_test["temperature"].mean())
    # Facteur humidité
    humid_factor = 1.0 - 0.0001 * (df_test["humidity"] - df_test["humidity"].mean())
    ...
    for i, h in enumerate(heures):
        ...
        if saison == "Bas":
            if 0 <= h <= 4:          # nuit
                ...
                y_adj[i] *= 0.98
            elif 8 <= h <= 9:        # pointe matin faible
                y_adj[i] *= 1.15
            elif 10 <= h <= 12:      # journée calme
                y_adj[i] *= 1
            elif h == 13:      # journée calme
                y_adj[i] *= 0.9  
            elif 14 <= h <= 17:      # pointe soir modérée
                y_adj[i] *= 0.85   
            elif 18 <= h <= 21:      # pointe soir modérée
                y_adj[i] *= 1.06
            elif h >= 22:
                y_adj[i] *= 0.89
        ...
        # -------- Saison HAUTE --------
            ...
            if 0 <= h <= 4:
                ...
                y_adj[i] *= 0.8      
            elif 17 <= h <= 21:      # TRÈS forte pointe soir
                y_adj[i] *= 0.94
            elif h >= 22:
                y_adj[i] *= 0.90
